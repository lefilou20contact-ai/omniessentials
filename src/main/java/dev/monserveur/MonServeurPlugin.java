...
import dev.monserveur.auction.AuctionManager;
import dev.monserveur.build.SpawnBuilder;
import dev.monserveur.command.AuctionCommand;
import dev.monserveur.command.HomeCommands;
import dev.monserveur.command.NpcCommand;
import dev.monserveur.command.SellCommand;
import dev.monserveur.command.TeleportRequestCommands;
...
import dev.monserveur.teleport.TpaManager;
...
public final class MonServeurPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        bind("ah", new AuctionCommand(this));
        TeleportRequestCommands tpaCommands = new TeleportRequestCommands(this);
        for (String name : List.of("tpa", "tpahere", "tpaccept", "tpdeny", "tpacancel", "back")) {
            bind(name, tpaCommands);
        }
        ...
        // Vérifie les annonces de l'hôtel des ventes expirées, toutes les 5 minutes
        getServer().getScheduler().runTaskTimer(this, () -> auctions.checkExpired(), 20L * 60 * 5, 20L * 60 * 5);
